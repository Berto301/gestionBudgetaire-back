    ...
    register = async (req,res,next) => {
        try{
            ...
            if(!userData && (!groupData || !societyData)){
                return ResponseUtil.sendError(res,{message:"Please enter informations"}) 
            }

            let groupCreated ={} , societyCreated={} , groupUpdated = {}, societyUpdated={},userCreated={},userUpdated={}, data = {}
            if(groupData?.name){
                if(groupData?._id){
                    groupUpdated = await requestService.updateById(groupData?._id,groupData,Group) // update group
            ...
            if(societyData?.name){   
                if(groupData?._id){
                    societyUpdated = await requestService.updateById(societyData?._id,societyData,Society) // update society
            ...
            if(userData?._id){
                userUpdated = await requestService.updateById(dataUser?._id,dataUser,Users)
            ...
            if(data?.users && (data?.group || data?.society)){
                next()
                return ResponseUtil.sendSuccess(res,{message:"Account created",success:true,data})
            }
        ...
        try{
            try {
                ...
                if(!users){
                  ...
                  const isPasswordsSame = await bcrypt.compareSync(password, users.password); //compare password
                  if(isPasswordsSame){
                    let payload ={
                        id:users?._id,
